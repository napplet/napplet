...
- [ ] **MIG-01**: Each of the 9 `@napplet/nub-<domain>` packages keeps its published name and becomes a 1-line re-export of `@napplet/nub/<domain>` (zero behavioral change for existing consumers)
- [ ] **MIG-02**: Each deprecated package adds a top-of-README deprecation banner pointing to `@napplet/nub/<domain>` and states the removal milestone
...
- [ ] **DOC-01**: New `@napplet/nub` README covering install, 9-domain layout, subpath pattern (barrel vs granular), and tree-shaking contract
- [ ] **DOC-02**: Root README, `@napplet/core` README, `@napplet/shim` README, `@napplet/sdk` README updated to reference `@napplet/nub/<domain>` paths
