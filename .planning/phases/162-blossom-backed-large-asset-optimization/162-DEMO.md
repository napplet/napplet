...
Protocol boundary: NIP-5D and NIP-5A retain authority over sandboxing, manifest, aggregate, and capability negotiation. The loader and its resource mapping are non-normative private bytes in signed `index.html`; the only runtime capability used is the existing NAP-RESOURCE whole-Blob `resource.bytes`/`resource.bytesMany` surface. Build-time SSRF and redirect checks are local hardening, not new protocol obligations.
