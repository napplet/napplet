...
| Term | Definition |
|------|-----------|
| Shell | A web application hosting napplet iframes, acting as a NIP-01 pseudo-relay and NIP-07 signer proxy |
| Napplet | A sandboxed iframe application communicating with the shell via postMessage |
| Pseudo-relay | The shell's internal message router that speaks NIP-01 wire format over postMessage |
...
4. **Shell verifies AUTH:**
   ...
   c. `relay` tag MUST be `napplet://shell` (or the shell's configured pseudo-relay URI)
