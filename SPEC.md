...
| dTag | A derived identifier combining napplet pubkey prefix and napplet type |
...
4. **Shell verifies AUTH:**
   ...
   f. If `aggregateHash` is non-empty, the shell SHOULD verify it against the napplet's published NIP-5A manifest on relays. If verification fails, the shell MUST reject AUTH. If no manifest is found on relays, the shell SHOULD accept (dev mode).
