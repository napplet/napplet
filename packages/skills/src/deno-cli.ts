...
const HELP = renderCliHelp(Object.values(TARGETS));

function parse(
  argv: string[],
): { cmd?: string; positional: string[]; opts: InstallOptions } {
...
export function runCli(argv: string[]): number {
  ...
  if (
    !cmd || cmd === "help" || argv.includes("--help") || argv.includes("-h")
  ) {
    console.log(HELP);
    return 0;
  }
  try {
    ...
    if (cmd === "install") {
      ...
      const results = install({
        ...opts,
        skills,
        to: opts.dir || opts.out ? undefined : (opts.to ?? "claude"),
      });
      for (const result of results) {
        console.log(
          `${result.action.padEnd(10)} ${
            result.skill.padEnd(16)
          } → ${result.dest}`,
        );
      }
      console.log(
        `\n${results.length} file(s) for ${
          results.map((result) => result.skill).filter((value, index, all) =>
