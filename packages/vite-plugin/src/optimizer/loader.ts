...
export class ResourceRuntime {
  ...
  async resolveMany(sources: string[]): Promise<Blob[]> {
    ...
    for (const group of chunks(entries, this.maxBatchSize)) {
      ...
      if (pending.length > 0) {
        const requests = pending.map(({ entry }) => ({ url: entry.uri }));
        const items = await withTimeout(requireResource(this.runtimeWindow).bytesMany(requests), this.timeoutMs);
        if (items.length !== pending.length || items.some((item, index) => !item.ok || !item.blob || item.url !== requests[index]!.url)) {
