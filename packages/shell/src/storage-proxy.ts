/**
 * Storage proxy — shell-side handler for napp storage requests.
 *
 * Napps without allow-same-origin cannot access localStorage directly.
 * This proxy stores napp data in the shell's localStorage, scoped by
...
import { aclStore, DEFAULT_STORAGE_QUOTA } from './acl-store.js';

function scopedKey(pubkey: string, dTag: string, aggregateHash: string, userKey: string): string {
  return `napp-storage:${pubkey}:${dTag}:${aggregateHash}:${userKey}`;
}

function calculateNappStorageBytes(
  pubkey: string, dTag: string, aggregateHash: string, excludeUserKey?: string,
): number {
  const prefix = `napp-storage:${pubkey}:${dTag}:${aggregateHash}:`;
  const excludeScopedKey = excludeUserKey
    ? `napp-storage:${pubkey}:${dTag}:${aggregateHash}:${excludeUserKey}`
    : null;
  let totalBytes = 0;
...
function sendResponse(sourceWindow: Window, correlationId: string, tags: string[][]): void {
  const responseEvent: Partial<NostrEvent> = {
    ...
    tags: [['t', 'napp:storage-response'], ['id', correlationId], ...tags],
    content: '',
    id: '',
    sig: '',
  };
...
export function handleStorageRequest(
  ...
  switch (topic) {
    case 'shell:storage-get': {
      if (!key) { sendError(sourceWindow, correlationId, 'missing key tag'); return; }
      if (!aclStore.check(pubkey, dTag, aggregateHash, 'storage:read')) {
        sendError(sourceWindow, correlationId, 'storage:read capability denied'); return;
      }
    ...
    case 'shell:storage-set': {
      if (!key) { sendError(sourceWindow, correlationId, 'missing key tag'); return; }
      const value = event.tags?.find((t) => t[0] === 'value')?.[1] ?? '';
      if (!aclStore.check(pubkey, dTag, aggregateHash, 'storage:write')) {
      ...
      try {
        ...
        sendError(sourceWindow, correlationId, 'storage write failed');
      }
      break;
    }
    case 'shell:storage-remove': {
      if (!key) { sendError(sourceWindow, correlationId, 'missing key tag'); return; }
      if (!aclStore.check(pubkey, dTag, aggregateHash, 'storage:write')) {
        sendError(sourceWindow, correlationId, 'storage:write capability denied'); return;
      }
    ...
    case 'shell:storage-clear': {
      if (!aclStore.check(pubkey, dTag, aggregateHash, 'storage:write')) {
        sendError(sourceWindow, correlationId, 'storage:write capability denied'); return;
      }
      cleanupNappStorage(pubkey, dTag, aggregateHash);
      sendResponse(sourceWindow, correlationId, [['ok', 'true']]);
      break;
    }
    case 'shell:storage-keys': {
      if (!aclStore.check(pubkey, dTag, aggregateHash, 'storage:read')) {
        sendError(sourceWindow, correlationId, 'storage:read capability denied'); return;
      }
      const prefix = `napp-storage:${pubkey}:${dTag}:${aggregateHash}:`;
    ...
    default:
      sendError(sourceWindow, correlationId, `unknown storage topic: ${topic}`);
      break;
  }
}

export function cleanupNappStorage(pubkey: string, dTag: string, aggregateHash: string): void {
  const prefix = `napp-storage:${pubkey}:${dTag}:${aggregateHash}:`;
